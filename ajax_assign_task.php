 ...
 */
if ( kanpress_current_user_can( 'task_assign' ) ) {
	...
	if ( isset( $_POST[ 'taskId' ] ) ) {
		 ...
		 */
		$q_select = 'SELECT ID, display_name FROM ' . $wpdb->prefix . 'users WHERE ID = %d';
		...
		if ( !empty( $user ) ) {
			...
			die( get_avatar( $user[ 'ID' ], 50, null, $user[ 'display_name' ] ) );
		} else {
