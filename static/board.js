...
( function( $ ) {
	...
	Kanpress.countTasks = function() {
		$( '#col1, #col2, #col3' ).each(function( n, column ) {
			var $tasks = $( column ).find( '.tarea' ).length;
			$( column ).find( 'h3 span' ).html( '(' + $tasks + ')' );
		} );
	};
	...
	Kanpress.assignTask = function() {
		...
		.fail( function( jqxhr ) {
			if ( 403 === jqxhr.status ) {
				alert( KanpressData.l10n[ 'You don\'t have permission to assign tasks' ] );
			}
		} )
	...
	Kanpress.setTaskDragAndDrop = function() {
		...
		$( '.tasks-area' ).droppable( {
			...
			drop: function( event, ui ) {
				...
				var $oldColumn = $( '.ui-draggable-dragging' ).parent(),
				...
				( function( $oldColumn, newStatus, taskId, $taskDragging ) {
					...
					.fail( function( jqxhr ) {
						if ( 403 === jqxhr.status ) {
							$oldColumn.append( $taskDragging );
							alert( KanpressData.l10n[ 'You don\'t have permission to move tasks' ] );
						}
					} )
				...
				} )( $oldColumn, newStatus, taskId, $( '.ui-draggable-dragging' ) );
			}
		} );
	};
	...
	$( function() {
		...
		if ( KanpressData.userCapabilities[ 'kanpress_task_assign' ] ) {
