...
/**
 * Proxy for Kanpress-specific capabilities for the current user. This is useful
 * while we don't have custom core capabilities.
 * 
 * @param string $capability The Kanpress capability
 * @return boolean Whether the current user has the capability or not.
 */
function kanpress_current_user_can( $capability ) {
	
	$kanpress_to_native_capabilities = array(
		'task_assign'	=> 'edit_posts',
		'task_create'	=> 'edit_posts',
		'task_remove'	=> 'edit_posts',
		'task_move'		=> 'edit_posts',
		'task_link'		=> 'edit_posts',
		'task_edit'		=> 'edit_posts',
	);
	
	if ( !in_array( $capability, array_keys( $kanpress_to_native_capabilities ) ) ) {
		return null;
	}
	
	return current_user_can( $kanpress_to_native_capabilities[ $capability ] );
}
